...
def load_fake_person_df(records: int = 3000) -> DataFrame:
    """
    Generate a minimal DataFrame with just name and gender data using Faker.
    
    :param records: Number of fake person records to generate (default: 3000)
    :return: A DataFrame with fake person data containing only name and gender fields
    """
    fake = Faker()
    ...
    for _ in range(records):
        ...
        data.append({
            ...
            'first_name_male': first_name if gender == 'male' else '',
            'first_name_female': first_name if gender == 'female' else '',
            'last_name_male': last_name if gender == 'male' else '',
            'last_name_female': last_name if gender == 'female' else '',
            'prefix': 'Mr.' if gender == 'male' else random.choice(['Ms.', 'Mrs.']),
            'prefix_male': 'Mr.' if gender == 'male' else '',
            'prefix_female': random.choice(['Ms.', 'Mrs.']) if gender == 'female' else '',
        })
